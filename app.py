...
# Configurar página - DEBE SER LA PRIMERA LLAMADA DE STREAMLIT
st.set_page_config(
...
class OpenRouterClient:
    ...
    def generate_content(self, prompt):
        try:
            ...
            return "En este momento tengo dificultades técnicas. Por favor, intenta nuevamente."
...
def generar_resumen_interes(historial_conversacion, interes_seleccionado):
    try:
        ...
        if not model:
            return f"Cliente interesado en {interes_seleccionado}"
        
        prompt = f"""
        Resume en MÁXIMO 50 palabras los intereses del cliente:
        ...
        return f"Interesado en {interes_seleccionado}"
...
def generate_contextual_response(query, context_documents):
    try:
        model = load_openrouter_model()
        if not model:
            return "¡Te recomiendo contactar a nuestro equipo de ventas! 😊"
...
RESPUESTA BREVE:"""
            ...
            prompt = f"""Responde en 1-2 oraciones máximo. Sé breve y directo.
...
def generate_quick_response(query):
    quick_responses = {
        ...
        'sí': "¡Excelente! Vamos a registrar tus datos para contactarte. 📞",
        'si': "¡Excelente! Vamos a registrar tus datos para contactarte. 📞",
    }
        ...
        with st.form(key="contact_form_main", clear_on_submit=True):
            ...
            with col2:
                ...
                interes = st.selectbox(
                    ...
                    ["No especificado", "ESET PROTECT Elite", "ESET PROTECT Enterprise", "ESET PROTECT Complete", "ESET PROTECT Advanced", "ESET PROTECT Entry", "Detección y Respuesta", "Seguridad para Endpoints", "Otro"],
                    index=0
                )
...
Un especialista te contactará en 24 horas. ¡Estamos aquí para ayudarte! 🚀"""
                        ...
                        st.error("❌ Error al guardar. Por favor intenta nuevamente.")
